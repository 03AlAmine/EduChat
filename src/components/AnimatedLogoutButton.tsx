// components/AnimatedLogoutButton.tsx
import React, { useState } from "react";
import "./AnimatedLogoutButton.css";

const AnimatedLogoutButton: React.FC<{ onClick: () => void }> = ({ onClick }) => {
  const [animationState, setAnimationState] = useState<
    "idle" | "walking" | "falling" | "complete"
  >("idle");
  ...
  const handleClick = () => {
    ...
    // Séquence d'animation complète avant déconnexion
    setTimeout(() => {
      setAnimationState("falling");
      setTimeout(() => {
        setAnimationState("complete");
        onClick(); // Déclenche la déconnexion seulement ici
      }, 2000); // Durée de l'animation de chute
    }, 500); // Durée de l'animation de marche
  };
  ...
  return (
    <button
      className={`logoutButton logoutButton--dark ${
        ...
        animationState === "falling" || animationState === "complete"
          ? "falling"
          : ""
      }`}
      onClick={handleClick}
    ...
    >
      ...
      <span className="button-text">
        {animationState === "idle" ? "Log Out" : "Logging Out..."}
      </span>
    </button>
  );
};
